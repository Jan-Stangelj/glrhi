...
namespace glrhi {
    ...
    void gbuffer::m_loadShader() {
        ...
        const char* fragCode = "#version 460 core\n"
                                ...
                                "}\n"
                                "\n"
                                "// Box blur for SSAO stored in alpha channel\n"
                                "float blurSSAO(vec2 uv) {\n"
                                "    float result = 0.0;\n"
                                "    vec2 texelSize = 1.0 / vec2(textureSize(u_resoult, 0));"
                                "    for (int x = -2; x <= 2; ++x) {\n"
                                "        for (int y = -2; y <= 2; ++y) {\n"
                                "            vec2 offset = vec2(x, y) * texelSize;\n"
                                "            result += texture(u_resoult, uv + offset).a;\n"
                                "        }\n"
                                "    }\n"
                                ...
                                "    vec3 color = tex.rgb;\n"
                                "\n"
                                "    // Blur SSAO from alpha channel\n"
                                "    float ssao = blurSSAO(texUV);\n"
                                "\n"
                                "    // Apply SSAO to color (darken based on occlusion)\n"
                                "    color *= ssao;\n"
                                "\n"
